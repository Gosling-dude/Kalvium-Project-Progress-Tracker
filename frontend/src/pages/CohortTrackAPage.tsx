...
export function CohortTrackAPage() {
  ...
  return (
    <PageContainer>
      ...
      <div className="no-scrollbar flex w-full gap-1 overflow-x-auto rounded-lg border border-slate-200/80 bg-slate-100/70 p-1 sm:w-auto sm:self-start">
        {SUB_TRACKS.map((t) => {
          const count =
            ...
            >
              ...
              <span
                className={`inline-flex h-[1.125rem] min-w-[1.125rem] items-center justify-center rounded-full px-1 text-2xs font-semibold tabular ${
                  isActive ? "bg-brand-100 text-brand-700" : "bg-slate-200/80 text-slate-600"
                }`}
              >
                {count}
              </span>
